...
from generate_agents import (
    MODEL_ROUTING_PATH,
    SPEC_PATH,
    rendered_controller_skill,
    rendered_projections,
    rendered_packaged_omp_codex,
)
...
ProjectionCacheKey = tuple[str, str, str, tuple[str, ...], bool]
...
class ProjectionBundleCache:
    ...
    def __init__(self, routing_path: Path, targets: Sequence[str], *, mirror_omp: bool = False) -> None:
        self.routing_path = routing_path
        self.targets = tuple(targets)
        self.mirror_omp = bool(mirror_omp)
    ...
    def from_args(cls, args: argparse.Namespace) -> "ProjectionBundleCache":
        ...
        return cls(routing_path, targets, mirror_omp=bool(getattr(args, "codex_use_packaged_omp_default_routing", False)))
    ...
    def get(self) -> ProjectionBundle:
        ...
        key = (str(self.routing_path), routing_digest, package_digest, self.targets, self.mirror_omp)
        ...
        try:
            ...
            if self.mirror_omp:
                if "codex" not in self.targets:
                    raise InstallError("--codex-use-packaged-omp-default-routing requires --codex")
                mirror, mirror_meta = rendered_packaged_omp_codex(self.routing_path)
                projections["codex"] = mirror
                routing_meta = dict(routing_meta)
                routing_meta.update({
                    "model_routing_mode": "MIRROR_CANONICAL_OMP",
                    "codex_routing_mode": "MIRROR_CANONICAL_OMP",
                    "derived_policy_sha256": mirror_meta["derived_policy_sha256"],
                    "transformer_revision": mirror_meta["transformer_revision"],
                    "external_registry_sha256": mirror_meta["external_registry_sha256"],
                    "qualification_sha256": mirror_meta["qualification_sha256"],
                    "mirror_provider_counts": mirror_meta["provider_counts"],
                })
...
def automatic_verification_profile(args: argparse.Namespace) -> str:
    ...
    if bool(getattr(args, "codex_use_packaged_omp_default_routing", False)):
        if "codex" not in selected:
            raise InstallError("--codex-use-packaged-omp-default-routing requires --codex")
        if args.model_routing:
            raise InstallError("--codex-use-packaged-omp-default-routing cannot be combined with --model-routing")
    if selected == {"codex"}:
        return "codex"
    if selected == {"omp"}:
        return "omp"
...
def installation_targets(
    *,
    ...
    # Codex supports an isolated project home.  When explicitly supplied, use
    # it only for the Codex agent projection; BBK's package, manifest and other
    # harness roots remain scoped by the normal installer rules.
    configured_codex_home = os.environ.get("CODEX_HOME")
    if scope == "user":
        home = user_home()
        codex_root = Path(configured_codex_home).expanduser() if configured_codex_home else home / ".codex"
        return {
    ...
    codex_root = Path(configured_codex_home).expanduser() if configured_codex_home else project / ".codex"
    return {
...
def add_install_selection_flags(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--codex-use-packaged-omp-default-routing",
        action="store_true",
        help="derive Codex routes from packaged spec/model-routing.json OMP defaults (12 GPT, 6 DeepSeek Pro, 1 DeepSeek Flash)",
    )
    parser.add_argument(
        "--language-profiles",
        "--profiles",
