...
- Review only a named qualitative or cross-cutting product risk deterministic checks cannot establish. Consume current receipts and return `NO_MATERIAL_ASSURANCE_WORK` rather than duplicate mechanical checks when no qualifying question is supplied.
...
Additional procedures available on demand: `bbk-evidence`, `bbk-review-findings`, `bbk-solution-outcome-fit`, `bbk-implementation-structure`, `bbk-execution-slicing`, `bbk-state-decision-effect-design`, `bbk-review-context`, `bbk-review-intent`, `bbk-profile-routing`, `bbk-installed-profiles`, `bbk-context-routing`, `bbk-artifact`, `bbk-handoff`. Load one only when its method is material to the assigned responsibility.
...
Return one JSON object governed by `spec/schemas/role-returns/bbk-reviewer-return-v2.schema.json`. New returns use `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible through `spec/schemas/role-returns/bbk-reviewer-return-v1.schema.json`.
...
COMPACT uses `spec/schemas/role-results/bbk-reviewer-compact-result-v2.schema.json` and requires:
...
FULL uses the existing complete payload `spec/schemas/role-results/bbk-reviewer-result-v1.schema.json`. Use FULL when:
