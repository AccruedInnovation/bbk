...
- `bbk-phase-wayfinder` (canonical `bbk_phase_wayfinder`) — when one coherent capability increment needs detailed phase-level ownership, ordering, integration, and handoffs.
- `bbk-verification-designer` (canonical `bbk_verification_designer`) — when the work graph contains material claims whose assertions or evidence methods are not yet explicit.
- `bbk-worker-designer` (canonical `bbk_worker_designer`) — when a work-unit class needs a concrete least-privilege worker invocation and continuation contract.
- `bbk-reviewer` (canonical `bbk_reviewer`) — when an independent bounded review can expose planning gaps before authorization.
