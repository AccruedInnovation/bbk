...
<bbk-role-contract role="bbk_root_wayfinder" package-version="0.1.0-alpha.13.1">
...
Additional procedures available on demand: `bbk-plan`, `bbk-solution-outcome-fit`, `bbk-implementation-structure`, `bbk-execution-slicing`, `bbk-profile-routing`, `bbk-installed-profiles`, `bbk-state-decision-effect-design`, `bbk-review-plan`, `bbk-review-intent`, `bbk-procedure-design`, `bbk-context-routing`, `bbk-handoff`. Load one only when its method is material to the assigned responsibility.
...
Return one JSON object governed by `spec/schemas/role-returns/bbk-root-wayfinder-return-v1.schema.json`. Its common envelope is `spec/schemas/bbk-role-return-v1.schema.json` and its closed role payload is `spec/schemas/role-results/bbk-root-wayfinder-result-v1.schema.json`.

Use these exact discriminators:

- `schema`: `bbk.role-return.v1`
- `contract`: `bbk.root-wayfinder-return.v1`
- `role`: `bbk_root_wayfinder`
...
- `acceptance_ref` (REFERENCE; nullable) — Exact ask-backed or otherwise authority-bound acceptance reference; null when not accepted.
- `execution_authority_ref` (REFERENCE; nullable) — Exact standing or explicit execution-authority reference and covered effect classes; null when not authorized.
- `decision_refs` (REFERENCE_LIST) — Accepted, proposed, reopened, and pending decision references with authority and freshness state.
- `planning_state` (STRUCTURED) — Current territory/interface map, completed and active frontier, blockers, residual fog, dependencies, and invalidation state.
- `fit_refs` (REFERENCE_LIST) — Current SolutionOutcomeFit references or an explicit not-applicable disposition.
- `architecture_refs` (REFERENCE_LIST) — Current architecture and material-interface references or an explicit not-applicable disposition.
...
- `assurance_refs` (REFERENCE_LIST) — Current verification, evidence, and independent-review design references or an explicit not-applicable disposition.
- `invalidated_or_superseded_refs` (REFERENCE_LIST) — Planning objects invalidated, reopened, or superseded during this invocation.
...
`READY_TO_EXECUTE` may be returned only when every current input, required role-owned output, blocking dependency, evidence carrier, cleanup obligation, invalidation condition, and durable handoff required by this contract has been reconciled. The state authorizes only the next parent integration or assessment step named by the role contract.
