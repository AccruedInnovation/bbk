...
color: green
tools:
  - "Agent(bbk-territory-orchestrator, bbk-reviewer, bbk-worker, bbk-validator)"
  - "Read"
  - "Grep"
  - "Glob"
  - "Bash"
...
- `bbk-territory-orchestrator` (canonical `bbk_territory_orchestrator`) — when one current immutable TerritoryExecutionBoundary has satisfied its full entry contract, including coherent responsibility and completion semantics, closed WorkUnit and dependency membership, exact source and interface bindings, bounded authority and effect fence, workspace and resource isolation, quality and validation obligations, containment and recovery, qualified roles and tools, exact context, and a durable return contract.
- `bbk-reviewer` (canonical `bbk_reviewer`) — when the active baseline or current authorized execution direction contains one exact global, cross-territory, recovery, intent-conformance, proportionality, or completion-report claim that needs a distinct independent judgment beyond deterministic gates and candidate-bound validation, with a current bounded subject, exact assertions or questions, complete context and exposure policy, result schema, and finding-disposition route.
- `bbk-worker` (canonical `bbk_worker`) — when one exact executable WorkUnit has the four current blocking facts, direct root-owned orchestration is the shortest safe route, and no separate Worker Orchestrator aggregation is material.
- `bbk-validator` (canonical `bbk_validator`) — when one exact frozen candidate has one bounded deterministic assertion assignment that does not require multi-validator aggregation or a Validator Orchestrator.
