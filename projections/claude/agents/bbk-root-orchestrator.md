...
- `bbk-validator` (canonical `bbk_validator`) — when one exact read-only verified sealed `candidate-package-v1` has a tool-generated `contentSha256` admitted as its sole identity and one bounded deterministic assertion assignment that does not require multi-validator aggregation or a Validator Orchestrator.
