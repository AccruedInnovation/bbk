...
- `bbk-territory-orchestrator` (canonical `bbk_territory_orchestrator`) — when an admitted territory has satisfied its entry contract and can execute within a bounded dependency and authority envelope.
- `bbk-reviewer` (canonical `bbk_reviewer`) — when a global, cross-territory, recovery, intent-conformance, or completion claim needs an independent bounded review.
