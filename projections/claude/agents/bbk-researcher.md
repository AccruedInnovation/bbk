...
Compiled primary: `bbk-research`.
On demand: `bbk-context-routing`, `bbk-artifact`, `bbk-handoff`. Load only when material to this responsibility.
...
New returns: one JSON object governed by `spec/schemas/role-returns/bbk-researcher-return-v2.schema.json` in `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible only through `spec/schemas/role-returns/bbk-researcher-return-v1.schema.json`.
...
COMPACT `spec/schemas/role-results/bbk-researcher-compact-result-v2.schema.json` requires:
...
FULL `spec/schemas/role-results/bbk-researcher-result-v1.schema.json` applies when:
...
Readiness: `ANSWERED` requires the declared decision-relevant evidence threshold to be met for every blocking claim. A successful search, a plausible inference, or one source does not establish `ANSWERED` by itself.
