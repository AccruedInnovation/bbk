...
Return one JSON object governed by `spec/schemas/role-returns/bbk-prototyper-return-v2.schema.json`. New returns use `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible through `spec/schemas/role-returns/bbk-prototyper-return-v1.schema.json`.
...
COMPACT uses `spec/schemas/role-results/bbk-prototyper-compact-result-v2.schema.json` and requires:
...
FULL uses the existing complete payload `spec/schemas/role-results/bbk-prototyper-result-v1.schema.json`. Use FULL when:
