...
- `bbk-worker-orchestrator` (canonical `bbk_worker_orchestrator`) — when one candidate-producing cohort has admitted work units, non-overlapping ownership, tools, authority, and entry conditions.
- `bbk-validator-orchestrator` (canonical `bbk_validator_orchestrator`) — when an exact frozen candidate is eligible for its required assertion-scoped acceptance run.
- `bbk-reviewer` (canonical `bbk_reviewer`) — when a territory, integration, recovery, or readiness question needs an independent bounded review outside validator repair authority.
