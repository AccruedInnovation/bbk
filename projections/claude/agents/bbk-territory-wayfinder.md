...
- Project every substantive territory and territory-owned decision coordination record through mandatory `bbk-beads`; retain the canonical territory plan and decision authority in BBK and report tracker drift explicitly, preserving typed projection failures for deterministic correction.
...
On demand: `bbk-plan`, `bbk-solution-outcome-fit`, `bbk-implementation-structure`, `bbk-profile-routing`, `bbk-installed-profiles`, `bbk-state-decision-effect-design`, `bbk-review-plan`, `bbk-review-intent`, `bbk-procedure-design`, `bbk-context-routing`, `bbk-beads`, `bbk-artifact`, `bbk-handoff`. Load only when material to this responsibility.
...
New returns: one JSON object governed by `spec/schemas/role-returns/bbk-territory-wayfinder-return-v2.schema.json` in `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible only through `spec/schemas/role-returns/bbk-territory-wayfinder-return-v1.schema.json`.
...
COMPACT `spec/schemas/role-results/bbk-territory-wayfinder-compact-result-v2.schema.json` requires:
...
FULL `spec/schemas/role-results/bbk-territory-wayfinder-result-v1.schema.json` applies when:
...
Readiness: Use `READY_FOR_PARENT_INTEGRATION` only when no blocking internal frontier item remains, required child returns and governing sources are current, invalidation is resolved, boundary proposals and outward impacts are explicit, and residual uncertainty is proportionate.
