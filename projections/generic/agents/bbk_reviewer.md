<bbk-role-contract role="bbk_reviewer" package-version="0.1.0-alpha.13.1">
...
Primary procedure: `bbk-review`.
Mandatory procedures embedded below: `bbk-review`.
Additional procedures available on demand: `bbk-evidence`, `bbk-review-findings`, `bbk-solution-outcome-fit`, `bbk-implementation-structure`, `bbk-execution-slicing`, `bbk-state-decision-effect-design`, `bbk-review-context`, `bbk-review-intent`, `bbk-profile-routing`, `bbk-installed-profiles`, `bbk-context-routing`, `bbk-handoff`. Load one only when its method is material to the assigned responsibility.
...
Return one JSON object governed by `spec/schemas/role-returns/bbk-reviewer-return-v1.schema.json`. Its common envelope is `spec/schemas/bbk-role-return-v1.schema.json` and its closed role payload is `spec/schemas/role-results/bbk-reviewer-result-v1.schema.json`.

Use these exact discriminators:

- `schema`: `bbk.role-return.v1`
- `contract`: `bbk.reviewer-return.v1`
- `role`: `bbk_reviewer`
...
- `charter_manifest_and_context_refs` (REFERENCE_LIST) — Exact charter, AssuranceContract, ReviewManifest assignment and ReviewContextManifest or inline-context declaration as applicable; purpose, gate, assertions, lens, criteria, methods, included and excluded scope, context completeness, omissions, redactions, shards, cross-shard obligations and digests.
...
- `governing_sources_and_criteria` (STRUCTURED) — Accepted requirements, decisions, interfaces, plans, assertions, protected floors, applicability, criteria, methods, acceptance policy and source authority with exact identity, currentness and contradictions. Separate source-stated obligations, derived review implications and Reviewer inference.
- `context_completeness_and_omissions` (STRUCTURED) — Included, retrieval-only, excluded, omitted, redacted, generated, summarized, shared and sharded material; exact roots and pack digests; compiler; untrusted-content treatment; completeness; blockers; and effect on each conclusion.
- `independence_and_exposure_facts` (STRUCTURED) — Author, Worker, Validator and Reviewer role separation; logical and physical invocation; context-assembly independence; prior-findings visibility; outcome-bearing evidence exposure; model, provider, tool, environment, dataset and organizational diversity; candidate-mutation prohibition; attempt purpose; and every independence requirement satisfied, unsatisfied or not applicable.
- `inspection_actions_and_evidence` (STRUCTURED) — Every material inspection, command, tool, version, environment, input, output, EvidenceReceipt, trust class, reuse decision, coverage, redaction, limitation and distinction among direct observation, report, calculation and inference. Include exact path, byte count and SHA-256 where material.
- `assertion_evaluations` (STRUCTURED_LIST) — Every in-scope assertion or review question with exact result, applicability, criteria, rationale, evidence, coverage, confidence or uncertainty, limitations, primary or complementary ownership and claims explicitly not established.
...
- `out_of_scope_observations` (STRUCTURED_LIST) — Material concerns not evaluated under the current charter, why they are outside scope, available evidence, possible consequence, affected objects and proposed parent-owned charter or route. Do not assign a pass or fail beyond current authority.
...
- `profiles_tools_and_environments` (STRUCTURED) — Selected profile and router identities, effective digests, lenses, tools, evidence adapters, versions, environments, consumers, devices, credentials, qualifications, substitutions, capability gaps and impact on claim strength.
- `checkpoint_recovery_cleanup_and_residuals` (STRUCTURED) — Completed and remaining review work, exact checkpoint and resume prerequisites, same-attempt or successor rules, prior attempt state, interruption reason, scratch and sensitive material, temporary effects, cleanup or quarantine, sealed evidence retained and unresolved residual owners.
- `invalidated_or_superseded_refs` (REFERENCE_LIST) — Subjects, charters, contexts, criteria, evidence, findings, assessments, checkpoints or reports invalidated, reopened, replaced or superseded, exact cause and unaffected material eligible for reuse.
...
<bbk-inlined-skill name="bbk-review" source="spec/method-content.json#skills/bbk-review">
