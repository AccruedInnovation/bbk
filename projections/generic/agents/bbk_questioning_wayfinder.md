...
Use the installed BBK skills applicable to this invocation: `bbk`, `bbk-plan`, `bbk-solution-outcome-fit`, `bbk-procedure-design`, `bbk-context-routing`. Load only the procedure content needed for the current responsibility.
...
- `bbk_question_guide` — Resolve one declared material decision through a focused one-question-at-a-time conversation with a recommendation, explicit disposition, and ADR-compatible return packet.
