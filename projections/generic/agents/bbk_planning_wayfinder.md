...
- `bbk_phase_wayfinder` — when a defined phase needs phase-owned work-unit decomposition, internal ordering, mutation ownership, safe parallelism, integration touchpoints, checks, continuation, and handoffs; the phase may advance one or several capability increments and an increment may span several phases.
- `bbk_verification_designer` — when work-graph-wide, cross-phase, outcome, shared-interface, migration, recovery, or graph-level gate claims need exact assertions, methods, evidence, applicability, environments, independence rationale, or ownership deduplication before the graph can be sealed; phase-local claim identification, commissioning, return validation, graph integration, and readiness remain with the owning Phase Wayfinder, while exact assertion and evidence-method design remains Verification Designer-owned.
- `bbk_worker_designer` — when a reusable work-unit class, cross-phase integration unit, shared procedure or skill capsule, or plan-wide invocation standard needs one current exact least-privilege worker contract covering profiles, tools, authority, capability zones, isolation, runtime, continuation, payload, result, and handoff; phase-local WorkUnit identification, commissioning, return validation, graph integration, and readiness remain with the owning Phase Wayfinder, while exact invocation-contract design remains Worker Designer-owned.
- `bbk_reviewer` — when an exact bounded work-graph review charter and distinct independence reason can retire a material capability-ordering, dependency, integration, traceability, assertion-coverage, worker-readiness, authority, proportionality, or execution-feasibility risk before parent integration.
