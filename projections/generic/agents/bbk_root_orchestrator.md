<bbk-role-contract role="bbk_root_orchestrator" package-version="0.1.0-alpha.13.1">
...
Additional procedures available on demand: `bbk-recover`, `bbk-evidence`, `bbk-execution-slicing`, `bbk-implementation-structure`, `bbk-profile-routing`, `bbk-state-decision-effect-design`, `bbk-review-plan`, `bbk-context-routing`, `bbk-handoff`. Load one only when its method is material to the assigned responsibility.
...
Return one JSON object governed by `spec/schemas/role-returns/bbk-root-orchestrator-return-v1.schema.json`. Its common envelope is `spec/schemas/bbk-role-return-v1.schema.json` and its closed role payload is `spec/schemas/role-results/bbk-root-orchestrator-result-v1.schema.json`.

Use these exact discriminators:

- `schema`: `bbk.role-return.v1`
- `contract`: `bbk.root-orchestrator-return.v1`
...
- `planning_counterpart_ref` (REFERENCE) — Responsible Root Wayfinder, accepted planning handoff, operating-baseline acceptance reference, outstanding planning signals, and exact route for successor direction.
- `root_execution_charter_ref` (REFERENCE) — Root instruction package identity, compiler receipt, source checkpoint, qualified role and model/profile route, included and excluded concerns, budgets, stopping, cancellation, recovery, invalidation, completion, and return schema.
- `operating_baseline_ref` (REFERENCE) — Accepted operating-baseline identity, revision, digest, lifecycle, acceptance authority and handoff, or an exact blocking mismatch.
- `execution_baseline_ref` (REFERENCE) — Exact compiled execution-baseline identity, revision, digest, source-object manifest, TerritoryExecutionBoundary and WorkUnit mapping, instruction-package set, lifecycle, and current validity.
- `execution_authorization_ref` (REFERENCE) — Authority receipt, holder, exact baseline and campaign, scope, repositories, workspaces, environments, tools, resource ceiling, run and replacement allowance, external effects, residual risks, cancellation and recovery terms, expiry, revocation, and current status.
- `root_run_ref` (REFERENCE) — Canonical root semantic-run identity, lifecycle, current physical attempt, predecessor or replacement attempts, active lease and fencing state where available, startup-handshake receipt, host session bindings, checkpoint, and recovery envelope.
...
- `territory_boundary_inventory` (STRUCTURED) — Every admitted, pending, invalidated, completed, failed, cancelled, or superseded TerritoryExecutionBoundary with exact source territories and architecture elements, WorkUnit membership, dependencies, interfaces, integration obligations, authority, scope fences, resources, child run, recovery, and completion state.
- `dependency_and_integration_state` (STRUCTURED) — Global dependency graph, cross-boundary bindings, critical path, eligibility, barriers, integration owners, assembly points, compatibility and failure semantics, gates, impact sets, invalidated edges, and unresolved integration work.
- `active_children_and_attempts` (STRUCTURED) — Every current or historical direct-child Territory Orchestrator and Reviewer logical run, physical attempt, host session, lease, continuation, expected silence, status, last verified checkpoint, result, interruption, replacement, and immutable handoff.
- `resource_budget_and_schedule_state` (STRUCTURED) — Model, context, process, tool, environment, device, service, storage, network, cost, elapsed-time, run, retry, replacement, and concurrency budgets; shared-resource ownership; use, reservation, remaining capacity, deadlines, waits, backpressure, and exhausted ceilings.
- `authority_and_scope_fence_state` (STRUCTURED) — Root authority and effective semantic, resource, workspace, mutation, credential, network, device, external-system, and effect fences; child narrowing; local-discovery envelopes and permits; safeguards; expiry; revocation; violations; and current enforcement limitations.
- `substrate_and_host_health` (STRUCTURED) — Authoritative and projection substrate health, host and tool availability, degraded-operation mode, current capability impact, stale or untrusted state, expected-silence windows, liveness and useful-progress evidence, and recovery requirements.
...
- `assurance_and_evidence_state` (STRUCTURED) — Active AssuranceContract and assertion references, deterministic gates, Validator runs, assertion evaluations, EvidenceReceipts, environments, independence and exposure, evidence reuse and invalidation, operational observations, and exact claims established or not established.
- `finding_and_review_state` (STRUCTURED) — Immutable validation and review findings, correlation, affected assertions and objects, severity or consequence basis, repair route, dispositions, closure evidence, residual impact, reopening triggers, ReviewRuns, and outstanding independent-review obligations.
- `discoveries_and_deviations` (STRUCTURED) — Ordinary implied work, committed local discoveries and permits, repeated or cumulative discoveries, cross-boundary effects, contract conflicts, baseline deviations, affected objects, containment, calibration implications, and exact parent route.
- `signals_and_parent_directions` (STRUCTURED) — Durable execution signals and directions with identity, type, source run, baseline, campaign, boundary, WorkUnit, candidate, affected scope and objects, evidence, sequence, idempotency, delivery, owner, resolution, and resulting continuation or pause state.
- `invalidated_or_superseded_refs` (REFERENCE_LIST) — Baselines, boundaries, instructions, runs, attempts, candidates, evidence, findings, signals, directions, reviews, handoffs, or projections invalidated, reopened, cancelled, or superseded during the campaign, with exact cause and unaffected material retained.
- `recovery_and_interruption_state` (STRUCTURED) — Current recovery class, interruption reason and evidence, containment, fenced attempts, ambiguous effects, reconciled state, recovery package, continuation identity, replacement allowance, completed actions, unresolved actions, and next safe command.
- `cleanup_and_external_effect_state` (STRUCTURED) — Processes, packages, credentials, services, ports, locks, databases, workspaces, generated artifacts, devices, remote systems, deployments, migrations, publications, and other effects with pre-state, permits, receipts, cleanup, rollback, compensation, quarantine, residuals, and accountable owner.
...
- `outcome_evidence_and_claims_not_established` (STRUCTURED) — Outcome-level or operational evidence available after execution, its subject, environment, horizon, attribution and limits, plus explicit claims that execution completion does not establish, such as user acceptance, outcome achievement, compliance, safety, security, risk acceptance, or release.
...
<bbk-inlined-skill name="bbk-root-execution" source="spec/method-content.json#skills/bbk-root-execution">
# BBK Root Execution
