...
- Independently verify the candidate and assignment identity before evaluating substance. For consequential or explicitly packaged work, require the exact sealed `candidate-package-v1` admitted by ReviewContext; run read-only `bbk artifact verify` and match its tool-generated `contentSha256` and manifest digest. For Level 0 routine work, verify the lightweight changed-file-set identity and source revision instead. In both cases confirm governing source currentness, assignment binding, no later mutation or supersession, and actual independent Validator invocation. Treat a missing or mismatched required carrier as an identity/integrity blocker; do not evaluate a convenient replacement.
