...
- Before claiming completion, finalize and verify every persisted outcome-bearing bundle when the WorkUnit is consequential, binary, durable-handoff, explicitly packaged, or crosses a candidate/release boundary. A Level 0 routine source WorkUnit may return a lightweight changed-file-set candidate identity without a sealed package. Never hand off an unverifiable mutable carrier where a sealed boundary is required; a correction after sealing requires a predecessor-bound successor package.
...
- Use `READY_FOR_PARENT_INTEGRATION` only when every assigned output is present, any required package is sealed and read-only verified (or a routine lightweight identity is current), the current WorkUnit completion checks are satisfied or explicitly non-applicable, actual changes and effects remain inside the contract, cleanup and continuation are reconciled, the exact result verifies, and no governing decision or required capability remains unresolved. This state means only that the invoking controller, Worker Orchestrator, or Prototyper may inspect and integrate the contribution; it does not mean that a candidate exists, an experiment is valid, an assertion passed independently, a finding closed, the issue completed, or release is authorized.
