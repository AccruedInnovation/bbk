...
- Before claiming completion, finalize and verify every persisted outcome-bearing bundle with the BBK artifact package engine. Attach the exact sealed path, package identity and revision, profile, `contentSha256`, manifest digest, and seal or publication and verification receipts. Never hand off a mutable JSON file, draft directory, ordinary path, hand-written hash, or stale `final` file as durable evidence. A correction after sealing requires a predecessor-bound successor package, new revision and digest, and new verification; never edit the sealed predecessor.
...
- Use `READY_FOR_PARENT_INTEGRATION` only when every assigned output is present, every persisted outcome-bearing bundle is sealed and read-only verified with current receipts attached, the current WorkUnit completion checks are satisfied or explicitly non-applicable, actual changes and effects remain inside the contract, cleanup and continuation are reconciled, the exact result verifies, and no governing decision or required capability remains unresolved. This state means only that the invoking Worker Orchestrator or Prototyper may inspect and integrate the contribution; it does not mean that a candidate exists, an experiment is valid, an assertion passed independently, a finding closed, the issue completed, or release is authorized.
...
Compiled primary: `bbk-work-unit-execution`.
On demand: `bbk-context-routing`, `bbk-artifact`, `bbk-handoff`, `bbk-evidence`, `bbk-recover`, `bbk-implementation-structure`, `bbk-execution-slicing`, `bbk-profile-routing`, `bbk-installed-profiles`, `bbk-state-decision-effect-design`, `bbk-procedure-design`. Load only when material to this responsibility.
...
New returns: one JSON object governed by `spec/schemas/role-returns/bbk-worker-return-v2.schema.json` in `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible only through `spec/schemas/role-returns/bbk-worker-return-v1.schema.json`.
...
COMPACT `spec/schemas/role-results/bbk-worker-compact-result-v2.schema.json` requires:
...
FULL `spec/schemas/role-results/bbk-worker-result-v1.schema.json` applies when:
...
Readiness: `READY_FOR_PARENT_INTEGRATION` requires current governing sources and invocation; complete assigned outputs; exact in-scope artifact and effect accounting; every persisted outcome-bearing bundle finalized or explicitly sealed and read-only verified by the BBK artifact package engine with exact identity, manifest digest, and receipts attached; satisfied or explicitly non-applicable WorkUnit completion checks; no silent substitution, unauthorized effect, ownership collision, unresolved governing decision, required capability gap, ambiguous cleanup, or stale checkpoint; and a verified durable result and handoff. It authorizes only parent inspection and integration of the contribution.
