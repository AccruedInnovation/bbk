...
- Independently verify assurance eligibility even when a parent already admitted the run: run read-only `bbk artifact verify` against the exact sealed `candidate-package-v1`; admit only its tool-generated `contentSha256` as candidate identity; require its sealed path, package ID and revision, profile, manifest digest, seal or publication receipt, verification receipt, and handoff to be exact and current; require prerequisite attestations or eligibility evidence to be sufficient for the active contract; and confirm no ordinary subject mutation, digest change, supersession, source, context, integrity or identity ambiguity remains. Reject mutable workspaces, drafts, loose files, legacy flat manifests, archives, handoff-only references, ad hoc hashes, and stale or unverifiable packages before ReviewContext compilation or evaluator dispatch.
...
- Compile or verify one deterministic ReviewContextManifest and exact context pack per assignment. Bind its candidate only by the admitted sealed package `contentSha256`, with the verified package and manifest identities as prerequisites; bind full content roots or exact retrieval handles, source revisions, digests, omissions, exclusions, redactions, generated summaries, profile and tool context, prior-findings visibility, outcome-bearing evidence exposure, context budget, untrusted-content treatment and invalidation dependencies. Never compile or reuse ReviewContext for a mutable path, draft, changed digest, failed verification, or predecessor after successor admission. A required missing or stale item is a context blocker; it does not become a candidate finding or implicit waiver.
...
Compiled primary: `bbk-validation-orchestration`.
On demand: `bbk-recover`, `bbk-evidence`, `bbk-solution-outcome-fit`, `bbk-implementation-structure`, `bbk-execution-slicing`, `bbk-state-decision-effect-design`, `bbk-profile-routing`, `bbk-installed-profiles`, `bbk-review-plan`, `bbk-review-context`, `bbk-review-run`, `bbk-review-findings`, `bbk-review-intent`, `bbk-context-routing`, `bbk-artifact`, `bbk-handoff`. Load only when material to this responsibility.
...
New returns: one JSON object governed by `spec/schemas/role-returns/bbk-validator-orchestrator-return-v2.schema.json` in `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible only through `spec/schemas/role-returns/bbk-validator-orchestrator-return-v1.schema.json`.
...
COMPACT `spec/schemas/role-results/bbk-validator-orchestrator-compact-result-v2.schema.json` requires:
...
FULL `spec/schemas/role-results/bbk-validator-orchestrator-result-v1.schema.json` applies when:
