...
- Project every substantive root execution-state transition and compact handoff pointer through mandatory `bbk-beads`; keep execution authority, candidate identity, validation admission, completion, and release state canonical in BBK, and preserve typed projection failures without semantic-state corruption.
...
- `bbk_validator` — when one exact read-only verified sealed `candidate-package-v1` has a tool-generated `contentSha256` admitted as its sole identity and one bounded deterministic assertion assignment that does not require multi-validator aggregation or a Validator Orchestrator.
...
Compiled primary: `bbk-root-execution`.
...
New returns: one JSON object governed by `spec/schemas/role-returns/bbk-root-orchestrator-return-v2.schema.json` in `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible only through `spec/schemas/role-returns/bbk-root-orchestrator-return-v1.schema.json`.
...
COMPACT `spec/schemas/role-results/bbk-root-orchestrator-compact-result-v2.schema.json` requires:
...
FULL `spec/schemas/role-results/bbk-root-orchestrator-result-v1.schema.json` applies when:
