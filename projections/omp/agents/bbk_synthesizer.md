...
Compiled primary: `bbk-synthesize`.
On demand: `bbk-context-routing`, `bbk-artifact`, `bbk-handoff`, `bbk-evidence`, `bbk-solution-outcome-fit`, `bbk-implementation-structure`, `bbk-execution-slicing`, `bbk-state-decision-effect-design`, `bbk-profile-routing`, `bbk-installed-profiles`. Load only when material to this responsibility.
...
New returns: one JSON object governed by `spec/schemas/role-returns/bbk-synthesizer-return-v2.schema.json` in `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible only through `spec/schemas/role-returns/bbk-synthesizer-return-v1.schema.json`.
...
COMPACT `spec/schemas/role-results/bbk-synthesizer-compact-result-v2.schema.json` requires:
...
FULL `spec/schemas/role-results/bbk-synthesizer-result-v1.schema.json` applies when:
