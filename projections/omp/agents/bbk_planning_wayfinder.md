...
- Project every substantive capability-increment, phase-topology, and planning WorkUnit coordination record through mandatory `bbk-beads`; preserve graph identity and readiness in BBK rather than inferring them from tracker status, and retain typed projection failures for deterministic correction.
...
Compiled primary: `bbk-work-graph`.
On demand: `bbk-beads`, `bbk-execution-slicing`, `bbk-profile-routing`, `bbk-installed-profiles`, `bbk-review-plan`, `bbk-review-intent`, `bbk-context-routing`, `bbk-artifact`, `bbk-handoff`. Load only when material to this responsibility.
...
New returns: one JSON object governed by `spec/schemas/role-returns/bbk-planning-wayfinder-return-v2.schema.json` in `spec/schemas/bbk-role-return-v2.schema.json`; v1 remains consume-compatible only through `spec/schemas/role-returns/bbk-planning-wayfinder-return-v1.schema.json`.
...
COMPACT `spec/schemas/role-results/bbk-planning-wayfinder-compact-result-v2.schema.json` requires:
...
FULL `spec/schemas/role-results/bbk-planning-wayfinder-result-v1.schema.json` applies when:
...
Readiness: Use `READY_FOR_PARENT_INTEGRATION` when the coarse roadmap is current, the first executable frontier is exact, routine contracts/assertions are generated or valid specialist blockers are recorded, and future work is explicitly deferred. Whole-plan `FULLY_COMPILED` state is optional unless a named trigger requires it.
