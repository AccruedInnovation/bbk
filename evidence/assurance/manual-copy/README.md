...
For isolated DeepSeek Codex actors, use `tools/codex_ds_lifecycle.py` with an
explicit BBK role and `deepseek-v4-pro` or `deepseek-v4-flash` target. The
keyless lifecycle stores only the `DEEPSEEK_API_KEY` environment reference;
see [`docs/INSTALL.md`](docs/INSTALL.md#deepseek-codex-project-actors-keyless).
