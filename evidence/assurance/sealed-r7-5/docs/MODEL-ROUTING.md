...
## Explicit Codex external targets

The additive [`spec/codex-external-targets.json`](../spec/codex-external-targets.json)
registry records DeepSeek V4 Pro and V4 Flash as `EXPLICIT_ONLY` targets. Both
targets and their provider capabilities remain `CONFIGURED_UNQUALIFIED` and
disabled until a later, candidate-bound qualification flow establishes them.
The registry is activation-neutral: it does not alter this document's 19
canonical role routes, OMP profiles, OpenAI parent defaults, credentials, or
generated projections. The upstream MIT source and complete byte-level map are
retained under [`third_party/codex-deepseek-subagent/`](../third_party/codex-deepseek-subagent/).

The generator also emits an inactive, install-consumable Codex-only bundle at
[`projections/codex_ds/`](../projections/codex_ds/): one `deepseek-v4-pro` and
one `deepseek-v4-flash` directory, each containing all 19 canonical role TOMLs.
Every file declares `model_provider = "deepseek"`, the explicit model, the
Responses transport, and only the `DEEPSEEK_API_KEY` environment-key reference.
The bundle is generated and checked with `tools/generate_agents.py`; generation
does not read the key, call the provider, mutate user configuration, or provide
a fallback to the canonical Codex route. The authoritative bundle inventory and
input digests are in [`projections/codex_ds/manifest.json`](../projections/codex_ds/manifest.json).

## Packaged OMP default mirror

Pass `--codex-use-packaged-omp-default-routing` with a Codex install to derive
all 19 Codex routes from the read-only packaged `spec/model-routing.json` OMP
table. The transform yields 12 native GPT routes, 6 DeepSeek Pro routes and 1
DeepSeek Flash route, records `MIRROR_CANONICAL_OMP` in install metadata, and
rejects a simultaneous `--model-routing` source before writing. No runtime OMP
state or credential value is read; omitting the flag preserves existing Codex
role bytes.
