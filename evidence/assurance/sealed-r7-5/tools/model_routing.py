...
TRANSFORMER_REVISION = "omp-codex-mirror-v1"


def compile_packaged_omp_defaults(
    routing: Mapping[str, Any] | Path | None = None,
    external_registry: Mapping[str, Any] | Path | None = None,
    qualification: Mapping[str, Any] | Path | None = None,
    *,
    root: Path = ROOT,
) -> dict[str, Any]:
    """Derive the provider-aware Codex policy from packaged OMP defaults.

    The operation is pure: it reads the canonical policy and the activation-
    neutral external-target registry, performs no credential or network
    access, and returns a deterministic effective policy plus identity data.
    ``openai-codex/<model>`` routes become native Codex models while
    ``deepseek/<target>`` routes retain a provider marker consumed by the
    Codex renderer.  Unknown or unresolved prefixes fail closed.
    """
    if routing is None:
        routing = DEFAULT_PATH
    if isinstance(routing, Path):
        source_path = routing
        routing = load_model_routing(routing, root=root)
    else:
        source_path = None
    # Keep path/source identity separate from the loaded mapping.
    if isinstance(routing, Mapping):
        canonical = dict(routing)
    else:  # pragma: no cover - guarded by the branches above
        canonical = dict(routing)
    if source_path is None:
        source_path = DEFAULT_PATH
    source_bytes = source_path.read_bytes() if isinstance(source_path, Path) and source_path.is_file() else canonical_json_bytes(canonical)
    source_digest = hashlib.sha256(source_bytes).hexdigest()

    def _load_json(value: Mapping[str, Any] | Path | None, fallback: Path) -> tuple[dict[str, Any], str]:
        if value is None:
            path = fallback
            raw = path.read_bytes()
        ...
        if isinstance(value, Path):
            ...
            return json.loads(raw.decode("utf-8")), hashlib.sha256(raw).hexdigest()
        obj = dict(value)
        return obj, hashlib.sha256(canonical_json_bytes(obj)).hexdigest()

    registry, registry_digest = _load_json(
        external_registry, root / "spec" / "codex-external-targets.json"
    )
    qualification_obj, qualification_digest = _load_json(
        qualification, root / "evidence" / "qualification" / "deepseek-codex-provider-seam-r4" / "qualification-receipt.json"
    )
    # Resolve target records without ever reading a credential value.  The
    # resolver enforces explicit-only targets and qualification freshness.
    try:
        from codex_external_targets import resolve_target
    except ImportError as exc:  # pragma: no cover
        raise ModelRoutingError(f"external-target resolver unavailable: {exc}") from exc

    direct = direct_routes(canonical)
    roles: dict[str, dict[str, Any]] = {}
    provider_routes: dict[str, dict[str, Any]] = {}
    for role_name in sorted(direct):
        route = direct[role_name]
        omp = route["omp"]
        omp_model = str(omp.get("model", ""))
        effort = str(omp.get("thinkingLevel", ""))
        if effort not in {"low", "medium", "high", "max"}:
            raise ModelRoutingError(f"unsupported OMP thinkingLevel for {role_name}: {effort!r}")
        if omp_model.startswith("openai-codex/"):
            model = omp_model.split("/", 1)[1]
            provider_routes[role_name] = {"provider": "openai-codex", "model": model}
        elif omp_model.startswith("deepseek/"):
            target_id = omp_model.split("/", 1)[1]
            ...
            if resolved.get("status") != "RESOLVED":
                rejection = resolved.get("rejection") or {}
                raise ModelRoutingError(
                    f"unknown or unresolved DeepSeek target {target_id!r}: "
                    f"{rejection.get('reason_code', 'REJECTED')}"
                )
            target = resolved.get("target") or {}
            model = str(target.get("model") or target_id)
            provider_routes[role_name] = {
                "provider": "deepseek",
                "target_id": target_id,
                "model": model,
            ...
            raise ModelRoutingError(
                f"unsupported provider prefix in packaged OMP model for {role_name}: {omp_model!r}"
            )
        entry = {host: dict(route[host]) for host in HOST_SPECS}
        entry["codex"] = {"model": model, "model_reasoning_effort": effort}
        roles[role_name] = entry

    policy: dict[str, Any] = {
        "schema_version": "bbk.model-routing.v2",
        "description": "Derived Codex policy mirroring packaged canonical OMP defaults.",
        "roles": roles,
        "package_version": canonical.get("package_version", ""),
        "derived_mode": "MIRROR_CANONICAL_OMP",
        "codex_provider_routes": provider_routes,
    }
    policy["identity"] = {
        ...
        "external_registry_sha256": registry_digest,
        "qualification_sha256": qualification_digest,
        "derived_policy_sha256": hashlib.sha256(canonical_json_bytes(policy)).hexdigest(),
        "role_count": len(roles),
    }
    return {"policy": policy, "identity": policy["identity"]}


def compile_packaged_omp_policy(*args: Any, **kwargs: Any) -> dict[str, Any]:
    """Convenience wrapper returning only the derived v2 policy."""
    return compile_packaged_omp_defaults(*args, **kwargs)["policy"]
