"""Windows installer portability regression contracts."""
...
install = load_module("bbk_install_alpha92_split", "tools/install.py")
bbk = load_module("bbk_cli_alpha92_split", "tools/bbk.py")
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch.dict(os.environ, {"HOME": str(isolated)}, clear=False):
                ...
                with mock.patch.object(install.Path, "home", return_value=actual):
                    assert_same_path(self, install.user_home(), isolated)
    ...
    def test_bbk_home_has_explicit_precedence_over_home(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch.dict(os.environ, {"BBK_HOME": str(explicit), "HOME": str(conventional)}, clear=False):
                assert_same_path(self, install.user_home(), explicit)
