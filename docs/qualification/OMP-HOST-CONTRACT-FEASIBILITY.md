...
- `VER-021`: the pre-effect `tool_call` hook replaced one compact dispatch envelope with the exact full task input before OMP spawned the child: rewrite observed `true`, child started `true`, compact marker absent from child request `true`.
...
```text
python tools/qualification/omp_host_contract.py --omp <path-to-omp-16.4.8> --output evidence/qualification/omp-host-contract-linux.json --markdown docs/qualification/OMP-HOST-CONTRACT-FEASIBILITY.md
```
