...
#### DeepSeek Codex actor status

Project-scoped DeepSeek actors are managed with
`tools/codex_ds_lifecycle.py`. Every install, reinstall, or update requires an
explicit BBK role and either `deepseek-v4-pro` or `deepseek-v4-flash`; the
resolver never falls back to another target. `status` reports
`credential_state: ABSENT` when `DEEPSEEK_API_KEY` is not present and reports
`provider_state: CREDENTIAL_ABSENT` instead of implying that a provider call
succeeded. A supplied provider diagnostic is surfaced as `provider_state:
ERROR` and `provider_error`; it does not alter the selected target. These
states are diagnostics only, not proof of provider availability, acceptance,
or release readiness.

To opt into the packaged canonical OMP mirror for Codex, pass
`--codex-use-packaged-omp-default-routing` to `tools/setup.py` or
`tools/install.py` together with `--codex`. This derives the complete 19-role
mixed map (12 GPT, 6 DeepSeek Pro, 1 DeepSeek Flash) before any file write;
without the flag, default Codex bytes remain unchanged.
