...
if [ -n "${BBK_PYTHON:-}" ]; then
	exec "$BBK_PYTHON" -S -B -X utf8 "$SCRIPT_DIR/bbk_artifact.py" "$@"
elif command -v python3 >/dev/null 2>&1; then
	exec python3 -S -B -X utf8 "$SCRIPT_DIR/bbk_artifact.py" "$@"
elif command -v python >/dev/null 2>&1; then
	exec python -S -B -X utf8 "$SCRIPT_DIR/bbk_artifact.py" "$@"
else
