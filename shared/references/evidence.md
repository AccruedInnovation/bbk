...
Bind environment evidence to the exact node or subject, stable identity, source, time, method, scope, authority, confidence, and limitations. Never transfer an observation from one machine, account, network, repository, version, or environment to another merely because they are similar. For a multi-file deliverable, prefer `bbk artifact manifest` and `bbk artifact verify` over ad hoc hashing.
